...
Notes
-----
- MVP_13_RDA_validation_shuffle.py is same as MVP_13_RDA_validation.py but 
  I tweaked some filecount assertions, and for-loops skipped doing anything 
  with individual data and only used 'pooled' data
      - eg expected_num calculation in retrieve_offset_data - factor of 1 (pooled only) instead of 2 (ind + pooled)
      - eg iterating ['pooled'] instead of ['ind', 'pooled'] in create_histo_subplots, create_heatmap_subplots, create_slope_heatmap_subplots
      

Usage
-----
conda activate mvp_env
python MVP_13_RDA_validation_shuffle.py seed slimdir outerdir
...
def retrieve_offset_data(seed, factor=None):
    ...
    expected_num = 1*4*2*factor  # pool/ind * TRUE/FALSE/CAUSAL/NEUTRAL * structcorr/uncorr * factor
